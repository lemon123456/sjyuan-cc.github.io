...
![]({{ "/assets/img/agile/agile-development-summary-of-project-eums-workout.jpg" }})
...
>从图中可以看出，一个Iteration通常始于[IPM](#ipm)，止于[Showcase](#showcase)，其中每天都会发生的有[Standup](#standup)。[Pair](#pair)、[TDD](#tdd)、[Code Review](#code-review)则发生在每天的Coding中，通常情况下，[Story kick-off](#story-kick-off)之后，就进入了开发环节，Coding便开始了。[CI](#ci)属于基础设施，通常在一个名为Iteration0的迭代完成，也就是正是开发开始之前就应该完成[CI](#ci)的搭建。[Retro](#retro)则较长一段时间才进行的活动，根据实际情况，有1个月一次，也有2个月一次的。另外[Regular catch up with client](#regular-catch-up-with-client)也是因项目而异，EUMs是Daily，也有Weekly的，有些项目甚至没有这项活动。
...
![]({{ "/assets/img/agile/agile-development-summary-of-project-eums-kick-off.jpg" }})
